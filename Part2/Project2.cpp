...
// Date : 12/6/17
// Description : This program attempts to predict the type of  input test documents using probabilities. This project is working 100% correctly with the 3 different input sets.
...
int main()
{
 ...
 // ifstream input1("/Users/student/Desktop/Part2/ShakespeareData_train_50.txt");//doc1
  ...
  /* for(int m=0; m<words3.size();m++) {
    ...
    colTotal3.push_back(pushit1);
  }
  frac =0.0;
  int testcount3=0;
  for(int d=0;d<colTotal3.size();d++)
    {
      
      frac = (colTotal3[d] + 1)/(float)(doc3.size()+2);
      fractions3.push_back(frac);
      testcount3++;
      
      // cout<<fractions3[d]<<endl;
    }
  cout<<testcount3<<endl;
  
  
  for(int m=0; m< doc4.size();m++) {
    pushit2 = 0.0;
    for (int n = 0; n < dictionary.size(); n++) {
      if (feature4[n][m] == 1)
	pushit2++;
  ...
  frac = 0.0;
  int testcount4 = 0;
  for(int d=0;d<colTotal4.size();d++)
    {
      // frac =  colTotal[d]/doc1.size();
      frac = (colTotal4[d] + 1)/(float)(doc4.size()+2);
      fractions4.push_back(frac);
      testcount4++;
      
      // cout<<fractions2[d]<<endl;
    }
  cout<<testcount4<<endl;
  */  
  vector <int> testData;
  ...
  float probShake1 = 0.0;
  float probEliza1 =0.0;
  ...
  for (int i = 0; i<feature4.size(); i++)
  {
    ...
    for (int j = 0; j < dictionary.size(); j++)
    {
      if (feature4[i][j] == 1)
      {
        probShake1+=log(fractions1[j]);
        probEliza1+=log(fractions2[j]);


      } else
      {
        //1- (prob of word to total)
        probShake1+=log(1-fractions1[j]);
        probEliza1+=log(1-fractions2[j]);

      }

    }
    probShake1+=log(shakeTotal);
    probEliza1+=log(elizaTotal);

    if(probShake1<probEliza1)
      guessEliza++;
  ...
  cout<<endl;
  cout << "Total number of words in dictionary: " << dictionary.size() << endl;
  cout<<"Total sonnet in " << file1 << " are: "<<total1<<endl;
  cout<<"Total sonnet in " << file2 << " are: "<<total2<<endl;
  cout<<endl;
  cout<<"Total sonnet in " << file3 << " are: "<<total3<<endl;
  cout<<"Classification accuracy (" <<guessShake<<"/"<<total3<<")= "<<(guessShake/total3)*100<<endl;
  cout<<"Total sonnet in " << file4 << " are: "<<total4<<endl;
  cout<<"Classification accuracy (" <<guessEliza <<"/"<<total4<<")= "<<(guessEliza/total4)*100<<endl;
  cout<<endl;
  cout<<"Combined Classification accuracy ("<<guessShake+guessEliza<<"/"<<total3+total4<<")= "
      << ((guessShake+guessEliza)/(total3+total4))*100<<endl;
  //  cout << testCount0 << endl;
  //  cout << testCount1 << endl;
